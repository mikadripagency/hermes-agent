...
-- Durable completion-delivery ledger. New completion flows may create a
-- pending row before sending; ``notify-reconcile`` can also add an
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    """Return True when ``task_id`` is deliberately parked in ``blocked``.

    A ``blocked`` status can come from three different sources:

    * **Initially blocked** — ``create_task(initial_status="blocked")`` parks a
      human-ops card before any worker can claim it.  The ``"created"`` event
      records that explicit status.
    ...
    * **Circuit-breaker** — ``_record_task_failure`` tripped after
    ...
    The latest explicit lifecycle event distinguishes deliberate parking from
    circuit-breaker or direct-DB status changes.  An ``"unblocked"`` event
    releases both initially and subsequently blocked tasks.
    """
    row = conn.execute(
...
def enforce_max_runtime(
    ...
    *,
    ...
    Sends SIGTERM and advances ownership only after the worker/agent is gone. Emits a
    ...
    for row in rows:
        ...
        with task_run_lock(database_path(conn), tid, exclusive=True):
            with write_txn(conn):
                cur = conn.execute(
                    "UPDATE tasks SET status = 'running', claim_lock = NULL, "
                    "claim_expires = NULL, worker_pid = NULL, "
    ...
    with write_txn(conn):
        ...
        if existing is None:
            legacy = conn.execute(
                ...
                "AND task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ? "
                "AND notifier_profile = '' AND state = 'pending' "
                "AND receipt_id IS NULL",
                (int(event_id), task_id, platform, chat_id, thread_id),
            ).fetchone()
            if legacy is not None:
                conn.execute(
                    "UPDATE completion_deliveries SET notifier_profile = ?, "
                    "state = 'acknowledged', receipt_id = ?, acknowledged_at = ? "
                    "WHERE event_id = ? AND task_id = ? AND platform = ? "
                    "AND chat_id = ? AND thread_id = ? AND notifier_profile = '' "
                    "AND state = 'pending' AND receipt_id IS NULL",
                    (
                        notifier_profile,
                        receipt_id,
                        now,
...
def reconcile_completion_delivery(
    ...
    *,
    ...
    This path never sends a message and never accepts a caller-supplied receipt.
    It only records a completed event whose exact subscription route already
    carries a non-empty platform receipt. Repeating the same reconciliation is
    idempotent; any conflicting ledger row is rejected.
    """
    task_id = str(task_id or "").strip()
    platform = str(platform or "").strip().lower()
    chat_id = str(chat_id or "").strip()
    ...
    with write_txn(conn):
        ...
        version_expr = "t.handoff_version" if "handoff_version" in task_columns else "1"
        event = conn.execute(
            f"SELECT e.kind, t.status, {version_expr} AS handoff_version "
            "FROM task_events e JOIN tasks t ON t.id = e.task_id "
            "WHERE e.id = ? AND e.task_id = ?",
            (int(event_id), task_id),
        ).fetchone()
        if event is None:
            raise ValueError("completion event does not match the requested task")
        ...
        if existing is not None:
            ...
            if existing["state"] == "acknowledged" and actual == expected:
                retired = conn.execute(
                    "DELETE FROM completion_deliveries WHERE event_id = ? "
                    "AND task_id = ? AND platform = ? AND chat_id = ? "
                ...
                if retired:
                    _append_event(
                        conn,
                        task_id,
                        "completion_delivery_legacy_route_retired",
                        {
                            "completed_event_id": int(event_id),
                            "platform": platform,
                            "chat_id": chat_id,
                            "thread_id": thread_id,
                            "notifier_profile": notifier_profile,
                            "reason": "matching owned route already acknowledged",
                        },
                    )
                return message_id
...
def remove_notify_sub(
    ...
    *,
    ...
    ``pending`` forever. Keep both sides atomic, refuse an in-flight claim, and
    leave an explicit event for every route that was cancelled without a send.
    Calling this for an already-orphaned row is also supported so operators can
    repair historical rows without editing SQLite directly.
    """
    platform = str(platform or "").strip().lower()
    chat_id = str(chat_id or "").strip()
    thread_id = str(thread_id or "").strip()
    ...
    with write_txn(conn):
        ...
        if sub is not None and sub["pending_event_id"] is not None:
        ...
        for row in pending:
            ...
            cancelled = conn.execute(
                ...
                "AND state = 'pending' AND receipt_id IS NULL",
                (
                    int(row["event_id"]), task_id, platform, chat_id,
                    thread_id, str(row["notifier_profile"] or ""),
                ),
            )
