...
When ``kanban.max_in_progress_per_project`` is set, no single project (repo /
experiment lane, grouped by ``tasks.project_id``) gets more than N workers
running at once, even if the global ``max_in_progress`` / per-profile caps
would allow it. Tasks with no project (NULL ``project_id``) are exempt. Mirrors
